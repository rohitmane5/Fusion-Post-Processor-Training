...
properties = {
  ...
  jobNo: {
    title      : "Job number",
    description: "Specifies the job number to be included in the header of the program.",
    group      : "UserInfo",
    type       : "string",
    value      : "",
    scope      : "post"
...
function writeProgramHeader() {
  writeComment(conditional(getGlobalParameter("stock-material"), "stock material = " + getGlobalParameter("stock-material")));
  writeComment(conditional(getGlobalParameter("display-name"), "Programmer Name = " + getGlobalParameter("display-name")));
  writeComment(conditional(getGlobalParameter("job-description"), "Setup Name = " + getGlobalParameter("job-description")));
  writeComment(conditional(getProperty("jobNo"), "Job number= " + getProperty("jobNo")));
  writeComment("WCS number= " + getSection(0).wcs);
  if (getGlobalParameter("stock-type") == "box") {
    const stock = getGlobalParameter("stock");
    const nums = stock.match(/-?\d+/g).map(Number);
    ...
    writeComment("Stock dimensions= Length " + length + " , Width " + width + " , Height " + height);
