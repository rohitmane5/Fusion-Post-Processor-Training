...
var settings = {
  ...
  retract: {
    ...
    homeXY                    : {onIndexing:false, onToolChange:{axes:[Z, Y], singleLine:false}, onProgramEnd:{axes:[X, Y]}} // Specifies when the machine should be homed in X/Y. Sample: onIndexing:{axes:[X, Y], singleLine:false}
  },
  parametricFeeds: {
...
function writeToolCall(tool, insertToolCall) {
  ...
  writeStartBlocks(insertToolCall, function () {
    ...
    if (tool.manualToolChange) {
      ...
      writeComment("TOOL CHANGE TO T" + toolFormat.format(tool.number) + " - " + tool.getDescription());
      onCommand(COMMAND_LOAD_TOOL);
      if (getProperty("optionalStop") && insertToolCall && [TOOL_DRILL, TOOL_DRILL_CENTER, TOOL_DRILL_SPOT, TOOL_DRILL_BLOCK, TOOL_TAP_RIGHT_HAND, TOOL_TAP_LEFT_HAND].includes(tool.type)) {
